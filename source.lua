...
local SoftSyncFolder = "SoftSync"
local ConfigurationFolder = SoftSyncFolder.."/Configurations"
local ConfigurationExtension = ".ssync"
...
local SoftSync = game:GetObjects("rbxassetid://10804731440")[1]



if gethui then
	print("hi")
	SoftSync.Parent = CoreGui-- gethui()
elseif syn.protect_gui then 
	syn.protect_gui(SoftSync)
	SoftSync.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	SoftSync.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	SoftSync.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == SoftSync.Name and Interface ~= SoftSync then
			Interface.Enabled = false
			Interface.Name = "SoftSync-Old"
		end
	end
else
...
local Main = SoftSync.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

SoftSync.DisplayOrder = 100
...
local Notifications = SoftSync.Notifications

local SelectedTheme = Library.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = Library.Theme[ThemeName]
	for _, obj in ipairs(SoftSync:GetDescendants()) do
	...
	SoftSync.Main.BackgroundColor3 = SelectedTheme.Background or Library.Theme.Default.Background
	SoftSync.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar or Library.Theme.Default.BackgroundColor3
	SoftSync.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.Divider or Library.Theme.Default.Divider
	SoftSync.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar or Library.Theme.Default.Topbar
	SoftSync.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow or Library.Theme.Default.Shadow
	
	SoftSync.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor or Library.Theme.Default.TextColor
	SoftSync.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor or Library.Theme.Default.TextColor
	SoftSync.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor or Library.Theme.Default.TextColor

	SoftSync.Main.BackgroundTransparency = 0.25
...
function Library:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("SoftSync | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if SoftSync.Name == "SoftSync" then
...
function Library:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "SoftSync | Powered by Rayfield"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "SoftSync | Powered by Rayfield" then
		LoadingFrame.Version.Text = "SoftSync"
	end
	...
	pcall(function()
		...
		if not isfolder(SoftSyncFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(SoftSyncFolder.."/Discord Invites") then
			makefolder(SoftSyncFolder.."/Discord Invites")
		end
		if not isfile(SoftSyncFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then 
				writefile(SoftSyncFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"SoftSync RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(SoftSyncFolder.."/Key System") then
			makefolder(SoftSyncFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			if not Success then
				print("SoftSync | "..Settings.KeySettings.Key.." Error " ..tostring(Response))
			end
		end
		...
		if isfile(SoftSyncFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(SoftSyncFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				if KeyMain.Input.InputBox.Text == Settings.KeySettings.Key then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(SoftSyncFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("SoftSync | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("SoftSync | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					if Dropdown.Selected.Text ~= Option then
						...
						if not Success then
							...
							TweenService:Create(Dropdown.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 1}):Play()
							Dropdown.Title.Text = "Callback Error"
							print("SoftSync | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
							TweenService:Create(Dropdown.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then 
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("SoftSync | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("SoftSync | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("SoftSync | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
