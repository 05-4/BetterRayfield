local Release = "PRE-RELEASE"
local NotificationDuration = 6.5
local BetterRayfieldFolder = "BetterRayfield"
local ConfigurationFolder = BetterRayfieldFolder .. "/Configurations"
local ConfigurationExtension = ".confg"
...
local Library = {
    ...
    Theme = {
        Default = {
            ...
            Divider = Color3.fromRGB(65, 65, 65),
        ...
        Arc = {
            ...
            Divider = Color3.fromRGB(46, 50, 64),
            
            Background = Color3.fromRGB(64, 69, 82),
            Topbar = Color3.fromRGB(56, 60, 74),
            Shadow = Color3.fromRGB(26, 30, 44),
            
            NotificationBackground = Color3.fromRGB(56, 60, 74),
            NotificationActionsBackground = Color3.fromRGB(230, 230, 230),
            ...
            ElementBackground = Color3.fromRGB(198, 198, 198),
            ElementBackgroundHover = Color3.fromRGB(230, 230, 230),
            SecondaryElementBackground = Color3.fromRGB(26, 30, 44),
            ElementStroke = Color3.fromRGB(26, 30, 44),
            SecondaryElementStroke = Color3.fromRGB(40, 40, 40),
            ...
            InputBackground = Color3.fromRGB(31, 159, 71),
            InputStroke = Color3.fromRGB(19, 65, 31),
            PlaceholderColor = Color3.fromRGB(26, 30, 44)
        },
    }
}
...
elseif syn.protect_gui then
    syn.protect_gui(BetterRayfield)
    BetterRayfield.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
    BetterRayfield.Parent = CoreGui:FindFirstChild("RobloxGui")
else
    BetterRayfield.Parent = CoreGui
end
...
if gethui then
    for _, Interface in ipairs(gethui():GetChildren()) do
        if Interface.Name == BetterRayfield.Name and Interface ~= BetterRayfield then
            Interface.Enabled = false
            Interface.Name = "BetterRayfield-Old"
        end
    end
else
...
local Main = BetterRayfield.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

BetterRayfield.DisplayOrder = 100
...
local Notifications = BetterRayfield.Notifications

local SelectedTheme = Library.Theme.Default

function ChangeTheme(ThemeName)
    SelectedTheme = Library.Theme[ThemeName]
    for _, obj in ipairs(BetterRayfield:GetDescendants()) do
    ...
    BetterRayfield.Main.BackgroundColor3 = SelectedTheme.Background or Library.Theme.Default.Background
    BetterRayfield.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar or Library.Theme.Default.BackgroundColor3
    BetterRayfield.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.Divider or Library.Theme.Default.Divider
    BetterRayfield.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar or Library.Theme.Default.Topbar
    BetterRayfield.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow or Library.Theme.Default.Shadow
    
    BetterRayfield.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor or Library.Theme.Default.TextColor
    BetterRayfield.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor or Library.Theme.Default.TextColor
    BetterRayfield.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor or Library.Theme.Default.TextColor
    
    BetterRayfield.Main.BackgroundTransparency = 0.25
    ...
    for _, TabPage in ipairs(Elements:GetChildren()) do
        for _, Element in ipairs(TabPage:GetChildren()) do
            if Element.ClassName == "Frame" and Element.Name ~= "Placeholder" and Element.Name ~= "SectionSpacing" and Element.Name ~= "SectionTitle" then
                Element.BackgroundColor3 = SelectedTheme.ElementBackground or Library.Theme.Default.ElementBackground
                Element.UIStroke.Color = SelectedTheme.ElementStroke or Library.Theme.Default.ElementStroke
            end
        end
    end
end
...
local function LoadConfiguration(Configuration)
    ...
    table.foreach(Data, function(FlagName, FlagValue)
        if Library.Flags[FlagName] then
            spawn(function()
                if Library.Flags[FlagName].Type == "Colorpicker" then
                    Library.Flags[FlagName]:Set(UnpackColor(FlagValue))
                else
                    if Library.Flags[FlagName].CurrentValue or Library.Flags[FlagName].CurrentKeybind or Library.Flags[FlagName].CurrentOption ~= FlagValue then Library.Flags[FlagName]:Set(FlagValue) end
                end
            end)
        else
            Library:Notify({Title = "Flag Error", Content = "Unable to find '" .. FlagName .. "'' in the current script"})
        end
    end)
end
...
function Library:Notify(NotificationSettings)
    spawn(function()
        ...
        if NotificationSettings.Actions then
            for _, Action in pairs(NotificationSettings.Actions) do
                ...
                NewAction.MouseButton1Click:Connect(function()
                    ...
                    if not Success then
                        print("BetterRayfield | Action: " .. Action.Name .. " Callback Error " .. tostring(Response))
                    end
        ...
        if BetterRayfield.Name == "BetterRayfield" then
...
function Hide()
    Debounce = true
    Library:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightCtrl", Duration = 7})
...
function Library:CreateWindow(Settings)
    ...
    LoadingFrame.Title.Text = Settings.LoadingTitle or "BetterRayfield | Powered by Rayfield"
    LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
    if Settings.LoadingTitle ~= "BetterRayfield | Powered by Rayfield" then
        LoadingFrame.Version.Text = "BetterRayfield"
    end
    ...
    pcall(function()
        ...
        if not isfolder(BetterRayfieldFolder .. "/" .. "Configuration Folders") then
    ...
    if Settings.Discord then
        if not isfolder(BetterRayfieldFolder .. "/Discord Invites") then
            makefolder(BetterRayfieldFolder .. "/Discord Invites")
        end
        if not isfile(BetterRayfieldFolder .. "/Discord Invites" .. "/" .. Settings.Discord.Invite .. ConfigurationExtension) then
            if request then
                request({
            ...
            if Settings.Discord.RememberJoins then
                writefile(BetterRayfieldFolder .. "/Discord Invites" .. "/" .. Settings.Discord.Invite .. ConfigurationExtension, "BetterRayfield RememberJoins is true for this invite, this invite will not ask you to join again")
            end
        else
    ...
    if Settings.KeySystem then
        ...
        if not isfolder(BetterRayfieldFolder .. "/Key System") then
            makefolder(BetterRayfieldFolder .. "/Key System")
        end
        ...
        if Settings.KeySettings.GrabKeyFromSite then
            ...
            if not Success then
                print("BetterRayfield | " .. Settings.KeySettings.Key .. " Error " .. tostring(Response))
            end
        end
        ...
        if isfile(BetterRayfieldFolder .. "/Key System" .. "/" .. Settings.KeySettings.FileName .. ConfigurationExtension) then
            if readfile(BetterRayfieldFolder .. "/Key System" .. "/" .. Settings.KeySettings.FileName .. ConfigurationExtension) == Settings.KeySettings.Key then
                Passthrough = true
            end
        end
            ...
            KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
                if KeyMain.Input.InputBox.Text == Settings.KeySettings.Key then
                    ...
                    if Settings.KeySettings.SaveKey then
                        if writefile then
                            writefile(BetterRayfieldFolder .. "/Key System" .. "/" .. Settings.KeySettings.FileName .. ConfigurationExtension, Settings.KeySettings.Key)
                        end
                        Library:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
                    end
                else
        ...
        function Tab:CreateButton(ButtonSettings)
            ...
            Button.Interact.MouseButton1Click:Connect(function()
                ...
                if not Success then
                    ...
                    print("BetterRayfield | " .. ButtonSettings.Name .. " Callback Error " .. tostring(Response))
                    task.wait(0.5)
                    Button.Title.Text = ButtonSettings.Name
        ...
        function Tab:CreateInput(InputSettings)
            ...
            Input.InputFrame.InputBox.FocusLost:Connect(function()
                    ...
                    if not Success then
                        ...
                        print("BetterRayfield | " .. InputSettings.Name .. " Callback Error " .. tostring(Response))
                        task.wait(0.5)
                        Input.Title.Text = InputSettings.Name
        ...
        function Tab:CreateDropdown(DropdownSettings)
            ...
            local function RefreshOptions()
                for _, DropdownOption in ipairs(Dropdown.List:GetChildren()) do
                    if DropdownOption:IsA("Frame") then
            ...
            function DropdownSettings:Set(NewOptions)
                ...
                Dropdown.Selected.Text = NewOptions
                ...
                if not Success then
                    ...
                    Dropdown.Title.Text = "Callback Error"
                    print("BetterRayfield | " .. DropdownSettings.Name .. " Callback Error " .. tostring(Response))
                    task.wait(0.5)
                ...
                for _, droption in ipairs(Dropdown.List:GetChildren()) do
                    if droption.Name ~= NewOptions then
                    ...
                    elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then
                        ...
                        if not KeybindSettings.HoldToInteract then
                            ...
                            if not Success then
                                ...
                                print("BetterRayfield | " .. KeybindSettings.Name .. " Callback Error " .. tostring(Response))
                                task.wait(0.5)
        ...
        function Tab:CreateToggle(ToggleSettings)
            ...
            Toggle.Interact.MouseButton1Click:Connect(function()
                ...
                if not Success then
                    ...
                    print("BetterRayfield | " .. ToggleSettings.Name .. " Callback Error " .. tostring(Response))
                    task.wait(0.5)
                        ...
                        if SliderSettings.CurrentValue ~= NewValue then
                            ...
                            if not Success then
                                ...
                                Library:Notify({
                                    ...
                                    Content = SliderSettings.Name .. " | " .. tostring(Response),
                                    Image = 4483362458
                                })
                                print("BetterRayfield | " .. SliderSettings.Name .. " Callback Error " .. tostring(Response))
                                task.wait(0.5)
                                Slider.Title.Text = SliderSettings.Name
                                TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
...
function insertInterface()
    local interfaces = getgenv().interfaces or {}

    table.insert(interfaces, BetterRayfield)
    getgenv().interfaces = interfaces
...
function Library:Destroy()
    BetterRayfield:Destroy()
end

getgenv().DestroyAll = function()
    for _,interface in pairs(getgenv().interfaces or {BetterRayfield}) do
        interface:Destroy()
    end
    ...
    if CEnabled then
        pcall(function()
            if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
                ...
                Library:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
            end
        end)
    end
end
